...
def train_loss_and_metrics_step(logit_dict, label_dict, global_step, scope):
    with tf.variable_scope(scope):
        ...
        with tf.name_scope('loss'):
            ...
            loss = tf.nn.weighted_cross_entropy_with_logits(targets=label, logits=logit, pos_weight=5, name='loss')
        ...
        with tf.name_scope("metrics"):
            metrics = [loss, auc_train, merge_summary]
        return loss_total, metrics


def train_step(loss, train_op, train_metrics, step, global_step, sess, step_name):
    loss_total, auc_train, merge_summary = train_metrics
    _, loss_val, global_step_val, auc_train_, merge_summary_ = sess.run(
        [train_op, loss, global_step, auc_train, merge_summary])
    tf.logging.info(
