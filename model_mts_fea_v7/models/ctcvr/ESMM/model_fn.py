...
def model_block(features, label_dict, fc_generator, is_training, keep_prob, params):
    ...
    tf.logging.info("building features:")
    feats = []
    filter_names = seq_feat_list + ['seller_3d_noroom_noord_rate', 'seller_28d_noroom_noord_rate']
    for key in outputs_dict:
    ...
    def seq_convolution(seq, in_channels, out_channels, kernel_size, strides, training=True, name='conv'):
        ...
        convolution = tf.nn.avg_pool(seq_bn, ksize=kernel_size, strides=strides, padding='VALID',
    ...
    # B = label_click.get_shape().as_list()[0]
    # var_att_score = tf.squeeze(var_att_score, axis=-1)  # (B, L', P)
    # var_att_score = tf.reshape(var_att_score, [B, -1])
