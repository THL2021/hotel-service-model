...
def transport_model_fn(data_batch, global_step):
    ...
    # model
    logit_dict, label_dict = model_block(features, label_dict, fc_generator, FLAGS.is_training, FLAGS.keep_prob,
                                         tf.constant(100000, dtype=tf.int32), params)
    logit_noroom = logit_dict['logit_noroom']

    with tf.variable_scope('predict'):
        # for rank service 2.0
        p_logit = tf.sigmoid(logit_noroom)
