...
def model_block(features, label_dict, fc_generator, is_training, keep_prob, global_step, params):
    ...
    with tf.variable_scope('logit'):
        ...
        with tf.variable_scope('mts_extractor'):
            ...
            filter_width = 7
            out_channels = 4
            filter_num = len(seq_names)
            filter_shape = [filter_num, filter_width, in_channels, out_channels]
            filter_stride = 3
            output_width = int(math.ceil(1.0 * (in_width - filter_width + 1) / filter_stride))
            filters = tf.get_variable("seq_filters",
                                      shape=filter_shape,
                                      collections=[dnn_parent_scope, ops.GraphKeys.GLOBAL_VARIABLES,
                                                   ops.GraphKeys.MODEL_VARIABLES])
            seq_conv_list = list()
            for i in range(len(seq_names)):
                ...
                seq_filter = tf.nn.embedding_lookup(filters, i)
                seq_bn = layers.batch_norm(seq_feats[seq_name], is_training=is_training, activation_fn=None,
                                           variables_collections=[dnn_parent_scope])
                convolution = tf.nn.conv1d(seq_bn, filters=seq_filter, stride=filter_stride, padding='VALID',
                                           name='{}_conv'.format(seq_name))  # (batch, in_width, out_channels)
                seq_conv_list.append(convolution)  # (B, L', J), J = out_channels
                add_tensor_summary(convolution, '{}_conv'.format(seq_name))

            conv_seqs = tf.stack(seq_conv_list, axis=3)  # (B, L', J, P)
            ...
            var_att_logit = layers.fully_connected(var_att_input, len(seq_names), activation_fn=tf.nn.relu, scope='v_att2',
                                                   weights_initializer=initializer,
