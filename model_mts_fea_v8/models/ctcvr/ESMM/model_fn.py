...
def model_block(features, label_dict, fc_generator, is_training, keep_prob, global_step, params):
    ...
    with tf.variable_scope('logit'):
        ...
        with tf.variable_scope('mts_extractor'):
            ...
            conv_len1 = seq_repr1.get_shape().as_list()[1]
            conv_len2 = seq_repr2.get_shape().as_list()[1]
            conv_len3 = seq_repr3.get_shape().as_list()[1]
            conv_len4 = seq_repr4.get_shape().as_list()[1]
            ...
            mtl_att_vector_list = list()

            time_att_input_raw = tf.transpose(seq_repr1, perm=[0, 2, 1])  # (B, J, L')
            time_att_input = layers.fully_connected(time_att_input_raw, 1, activation_fn=tf.nn.relu, scope='t1_att1',
                                                    weights_initializer=initializer,
                                                    variables_collections=[dnn_parent_scope])  # (B, J, 1)
            time_att_input = tf.squeeze(time_att_input, axis=-1)  # (B, J)
            time_att_logit = layers.fully_connected(time_att_input, conv_len1, activation_fn=tf.nn.relu, scope='t1_att2',
                                                    weights_initializer=initializer,
            ...
            mtl_att_vector1 = tf.squeeze(time_attention, axis=-1)  # (B, J)
            mtl_att_vector_list.append(mtl_att_vector1)

            time_att_input_raw = tf.transpose(seq_repr2, perm=[0, 2, 1])  # (B, J, L')
            time_att_input = layers.fully_connected(time_att_input_raw, 1, activation_fn=tf.nn.relu, scope='t2_att1',
                                                    weights_initializer=initializer,
                                                    variables_collections=[dnn_parent_scope])  # (B, J, 1)
            time_att_input = tf.squeeze(time_att_input, axis=-1)  # (B, J)
            time_att_logit = layers.fully_connected(time_att_input, conv_len2, activation_fn=tf.nn.relu, scope='t2_att2',
                                                    weights_initializer=initializer,
                                                    variables_collections=[dnn_parent_scope])  # (B, L')
            time_att_score = tf.nn.softmax(time_att_logit)  # (B, L')
            time_att_score = tf.expand_dims(time_att_score, axis=2)  # (B, L', 1)
            time_attention = tf.matmul(time_att_input_raw, time_att_score)  # (B, J, 1)
            mtl_att_vector2 = tf.squeeze(time_attention, axis=-1)  # (B, J)
            mtl_att_vector_list.append(mtl_att_vector2)

            time_att_input_raw = tf.transpose(seq_repr3, perm=[0, 2, 1])  # (B, J, L')
            time_att_input = layers.fully_connected(time_att_input_raw, 1, activation_fn=tf.nn.relu, scope='t3_att1',
                                                    weights_initializer=initializer,
                                                    variables_collections=[dnn_parent_scope])  # (B, J, 1)
            time_att_input = tf.squeeze(time_att_input, axis=-1)  # (B, J)
            time_att_logit = layers.fully_connected(time_att_input, conv_len3, activation_fn=tf.nn.relu, scope='t3_att2',
                                                    weights_initializer=initializer,
                                                    variables_collections=[dnn_parent_scope])  # (B, L')
            time_att_score = tf.nn.softmax(time_att_logit)  # (B, L')
            time_att_score = tf.expand_dims(time_att_score, axis=2)  # (B, L', 1)
            time_attention = tf.matmul(time_att_input_raw, time_att_score)  # (B, J, 1)
            mtl_att_vector3 = tf.squeeze(time_attention, axis=-1)  # (B, J)
            mtl_att_vector_list.append(mtl_att_vector3)

            time_att_input_raw = tf.transpose(seq_repr4, perm=[0, 2, 1])  # (B, J, L')
            time_att_input = layers.fully_connected(time_att_input_raw, 1, activation_fn=tf.nn.relu, scope='t4_att1',
                                                    weights_initializer=initializer,
                                                    variables_collections=[dnn_parent_scope])  # (B, J, 1)
            time_att_input = tf.squeeze(time_att_input, axis=-1)  # (B, J)
            time_att_logit = layers.fully_connected(time_att_input, conv_len4, activation_fn=tf.nn.relu, scope='t4_att2',
                                                    weights_initializer=initializer,
                                                    variables_collections=[dnn_parent_scope])  # (B, L')
            time_att_score = tf.nn.softmax(time_att_logit)  # (B, L')
            time_att_score = tf.expand_dims(time_att_score, axis=2)  # (B, L', 1)
            time_attention = tf.matmul(time_att_input_raw, time_att_score)  # (B, J, 1)
            mtl_att_vector4 = tf.squeeze(time_attention, axis=-1)  # (B, J)
            mtl_att_vector_list.append(mtl_att_vector4)

            mtl_att_vector = tf.concat(mtl_att_vector_list, axis=1)
