...
def model_block(features, label_dict, fc_generator, is_training, keep_prob, global_step, params):
    ...
    def build_resnet(input_seq, n_feature_maps):
        print ('build conv_x')
        x = input_seq  # keras.layers.Input(shape=(input_shape))
    ...
    with tf.variable_scope('logit'):
        ...
        with tf.variable_scope('mts_extractor'):
            ...
            keras.backend.set_learning_phase(is_training)
            mtl_att_vector = build_resnet(seq_features, 16)
            print('mtl_att_vector:', mtl_att_vector)
